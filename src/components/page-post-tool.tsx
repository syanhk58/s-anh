...
export default function PagePostTool() {
    // ─── Facebook Pages (có token, dùng để đăng bài) ──────────────────────────
    const [fbPages, setFbPages] = useState<FbPage[]>([]);
    const [selectedFbPage, setSelectedFbPage] = useState<FbPage | null>(null);
    ...
    // Filtered FB pages
    const filteredFbPages = fbPages.filter(p => {
        ...
        (async () => {
            try {
                ...
                if (data.fbPages && data.fbPages.length > 0) {
                    ...
                    setSelectedFbPage(data.fbPages[0]);
                }
            } catch { /* ignore */ }
    ...
    const canPost = selectedFbPage && (message.trim() || media.length > 0);
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-5">
            ...
            {/* ═══ CHỌN PAGE (từ Facebook Graph API) ═══ */}
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-5">
                <div className="flex items-center gap-2.5 mb-4">
                    ...
                    <h3 className="text-sm font-bold text-slate-700">① Chọn Page đăng bài</h3>
                    <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-600 font-bold">{fbPages.length} pages có token</span>
                </div>

                {loadingPages ? (
                    <div className="flex items-center gap-2 text-slate-400 py-4 justify-center">
                        <Loader2 className="h-4 w-4 animate-spin" />
                        <span className="text-xs">Đang tải danh sách pages...</span>
                    </div>
                ) : fbPages.length === 0 ? (
                    <div className="rounded-xl bg-amber-50 border border-amber-200 p-4 flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-amber-600 mt-1">Token Graph API chưa có hoặc hết hạn. Tạo lại token với scope <code className="bg-amber-100 px-1 rounded">pages_manage_posts</code></p>
                        </div>
                    </div>
                ) : (
                    <div className="relative" ref={pageDropdownRef}>
                        <button onClick={() => setPageDropdownOpen(!pageDropdownOpen)}
                            ...
                            {selectedFbPage?.picture ? (
                                <img src={selectedFbPage.picture} alt="" className="w-8 h-8 rounded-lg object-cover border border-white shadow-sm" />
                            ) : (
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="text-sm font-bold text-slate-700 truncate">{selectedFbPage?.name || "Chọn page..."}</div>
                                {selectedFbPage && <div className="text-[10px] text-slate-400">ID: {selectedFbPage.id}</div>}
                            </div>
                        ...
                        {pageDropdownOpen && (
                            <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl border border-slate-200 shadow-xl z-50 max-h-72 overflow-hidden">
                                ...
                                <div className="max-h-56 overflow-y-auto p-1">
                                    {filteredFbPages.length === 0 ? (
                                        <div className="text-center py-3 text-xs text-slate-400">Không tìm thấy page</div>
                                    ) : (
                                        filteredFbPages.map(p => (
                                            <button key={p.id} onClick={() => { setSelectedFbPage(p); setPageDropdownOpen(false); setPageSearch(""); }}
                                                className={cn("w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all text-left",
                                                    selectedFbPage?.id === p.id ? "bg-blue-50 border border-blue-200" : "hover:bg-slate-50")}>
                                                {p.picture ? (
                                                ...
                                                {selectedFbPage?.id === p.id && <Check className="h-4 w-4 text-blue-500 shrink-0" />}
                                            </button>
                                        ))
                                    )}
                                </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-5">
                ...
                <div className="rounded-xl border border-slate-200 bg-slate-50/30 p-4 mb-4">
                    <div className="flex items-center gap-3 mb-3">
                        ...
                        <div>
                            <div className="text-sm font-bold text-slate-700">{selectedFbPage?.name || "Chưa chọn page"}</div>
                ...
                <button onClick={handlePost} disabled={!canPost || postStatus === "loading"}
                    ...
                    {postStatus === "loading" ? (
                        ...
                        <><Send className="h-4 w-4" /> Đăng bài lên {selectedFbPage?.name || "Page"}</>
                    )}
                </button>
            </div>
        </div>
