                ...
                {loadingPages ? (
                    ...
                    <div className="rounded-xl bg-amber-50 border border-amber-200 p-4 flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-amber-600 mt-1">Kiểm tra lại token Graph API trong config. Cần scope <code className="bg-amber-100 px-1 rounded">pages_manage_posts</code></p>
                        </div>
                    </div>
                ) : (
                    <div className="relative" ref={dropdownRef}>
                        <button onClick={() => setShowPageDropdown(!showPageDropdown)}
                            className={cn("w-full flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all text-left",
                                showPageDropdown ? "border-blue-400 bg-blue-50/30 shadow-sm" : "border-slate-200 bg-slate-50/50 hover:border-slate-300")}>
                            {selectedPage ? (
                                <>
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="text-[10px] text-slate-400">ID: {selectedPage.id}</div>
                                    </div>
                                    <ChevronDown className={cn("h-4 w-4 text-slate-400 transition-transform", showPageDropdown && "rotate-180")} />
                                </>
                            ) : (
                                <>
                                    ...
                                    <span className="text-sm text-slate-400 flex-1">Chọn page...</span>
                                    <ChevronDown className={cn("h-4 w-4 text-slate-400 transition-transform", showPageDropdown && "rotate-180")} />
                                </>
                            )}
                        </button>
                        ...
                        {showPageDropdown && (
                            <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl border border-slate-200 shadow-xl z-50 max-h-64 overflow-hidden">
                                ...
                                <div className="max-h-48 overflow-y-auto p-1">
                                    {filteredPages.map(p => (
                                        <button key={p.id} onClick={() => { setSelectedPage(p); setShowPageDropdown(false); setPageSearch(""); }}
                                            ...
                                            {selectedPage?.id === p.id && <Check className="h-4 w-4 text-blue-500" />}
                                        </button>
                                    ))}
                                </div>
                            </div>
                    ...
                    {mode === "adlib" && (
                        <div className="space-y-4">
                            <div className="flex gap-2">
                                ...
                                <button onClick={handleScrape} disabled={!adUrl.trim() || scraping}
                                    className={cn("flex items-center gap-2 px-5 py-3 rounded-xl text-sm font-bold transition-all",
                                        adUrl.trim() && !scraping ? "bg-gradient-to-r from-violet-600 to-purple-600 text-white shadow-lg shadow-violet-200/50 hover:shadow-violet-300/60" : "bg-slate-100 text-slate-300 cursor-not-allowed")}>
                            ...
                            <div className="rounded-xl bg-blue-50/50 border border-blue-100 p-3 text-[11px] text-blue-600">
                                ...
                                Nếu scrape không lấy được media, bạn có thể tải ảnh/video từ quảng cáo rồi upload thủ công bên tab Upload.
                            </div>
                        </div>
                    )}
                ...
                <button onClick={handlePost} disabled={!canPost || postStatus === "loading"}
                    ...
                    {postStatus === "loading" ? (
                        ...
                        <>
                            ...
                            Đăng bài lên {selectedPage?.name || "Page"}
                        </>
                    )}
                </button>
            </div>
